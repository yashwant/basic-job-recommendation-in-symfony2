...
/**
 * UserEducationsRepository
 *
 */
class UserEducationsRepository extends EntityRepository
{

    /**
     * 
     * @param string $schoolName
     * @return boolean|\AppBundle\Entity\College
     */
    public function addCollege($schoolName)
    {

        echo $schoolName;
        $em = $this->getEntityManager();
        $entity = $em->getRepository('AppBundle:College')->findOneBy(array('name' => $schoolName));
        if ($entity) {
            echo 'returning entity';
            return $entity;
        }
        $newCollege = new College();
        $newCollege->setName($schoolName);
...
//            echo $e->getTraceAsString();
     ...
     * @return boolean
     */
    public function bulkAdd($educations, User $user)
    {

        $em = $this->getEntityManager();
        try {
            foreach ($educations['values'] as $edu) {
                $usrEdu = $em->getRepository('AppBundle:UserEducations')->findOneBy(array('linkedinId' => $edu['id'], 'user' => $user->getId()));
                if (!$usrEdu) {
                    $newUserEdu = new UserEducations();
                    if (array_key_exists('degree', $edu)) {
                        $newUserEdu->setDegree($edu['degree']);
                    }
                    if (array_key_exists('fieldOfStudy', $edu)) {
                        $newUserEdu->setFieldOfStudy($edu['fieldOfStudy']);
                    }
                    if (array_key_exists('notes', $edu)) {
                        $newUserEdu->setNotes($edu['notes']);
                    }
                    if (array_key_exists('schoolName', $edu)) {
                        $college = $this->addCollege($edu['schoolName']);
    //                    var_dump($college); exit();
                        $newUserEdu->setCollege($college);
                    }
                    else
                    {
    //                    echo 'school name doesnot exiss'; exit();
                    }
                    $userRef = $em->getRepository('AppBundle:User')->find($user->getId());
                    $newUserEdu->setUser($userRef);
    //                    $newUserEdu->setStartDate($startDate);
    //                    $newUserEdu->setEndDate($endDate);
                    $em->persist($newUserEdu);
                    $em->flush();
                    $em->clear();
    //                echo 'new User edu added';
                } else {
    //                echo 'User Edu already added';
                }
            }
            return true;
        } catch (\Exception $e) {
